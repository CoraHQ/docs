...
<AccordionGroup>
  ...
  <Accordion title="How far back does Cora look when detecting Signals?">
    ...
    - **Gmail** — 30 days
    - **Gong** — 30 days
    - **Slack** — no lookback (messages only after the bot is in the channel)
    - **Intercom** — 30 days
  ...
  <Accordion title="I added Cora to Slack but I’m not seeing Signals.">
    For proactive Signals, Cora must be in the channel and the channel must be mapped on the account. External channels auto-link; you set the internal channel in the account's settings.

    If you only want Cora to answer questions about recent Slack activity, inviting it is enough. See [Integrate Slack with Cora](/integrations/slack) for the difference between chat, data sync, and Signals.
  </Accordion>
</AccordionGroup>
