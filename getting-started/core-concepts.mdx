...
A [skill](/agents/how-to-build-skills) is reusable instructions — what to do. An [agent](/agents/understanding-agents) is a saved workflow with a trigger and the rest of the config. Inside an agent, each step is a **task**. Each time an agent fires, that is an **agent run**.

**Artifacts** are the saved outputs from agents and skills — documents, dashboards, surveys, and similar deliverables. They live on the **Artifacts** page. The run is the execution; the artifact is what you keep.
...
Chat or Slack can run a skill, start an agent, or write an artifact; an agent can also run a skill. Most agents still fire from a signal or a schedule.
