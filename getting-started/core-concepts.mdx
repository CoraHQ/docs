...
A [skill](/agents/how-to-build-skills) is reusable instructions — what to do — that chat or an agent can invoke. An [agent](/agents/understanding-agents) is when those steps should run: a trigger and the rest of the config. Inside an agent, each step is a **task**. Each time an agent fires, that is an **agent run**.

**Chat** is the on-demand path: ask a question, invoke a skill or an agent, or ask Cora to write something down. Chat can do the work once; an agent is what you save so it can run again.
